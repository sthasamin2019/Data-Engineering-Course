...
def transform(oltp_row, lookups):
    ...
    for row in oltp_row:
        ...
        # vehicle_id is nullable in trips (e.g. a request cancelled before a
        # vehicle was ever matched) and fact_trips allows NULL for vehicle_key —
        # only look up and skip when the OLTP row actually has a value.
        vehicle_key = None
...
def load_fact_trips(conn, fact_data):
 ...
 INSERT INTO fact_trips
    (source_trip_id, date_key, time_key, driver_key, passenger_key,
     ...
     payment_method_key, promo_code_key, vehicle_key,
